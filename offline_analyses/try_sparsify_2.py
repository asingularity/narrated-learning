...
def get_sparse_features(arr):
    ...
    changes from try_sparsify.py:

    (1) computes best RF to append each bin to in a greedy way, not per-RF

    (2) evaluates value of the RF while allowing overlap

    need to make sure:
    - don't add false positive bins back to remainder, so long as reconstruction is still getting better
    ...
    remainder_data_set = arr.copy()

    rf_patterns = np.zeros((num_rfs, feature_len))
    rf_values = np.zeros(num_rfs)  # i.e. net explanatory value, as average time-integral of bins explained. with erroneous bins subtracted

    # TODO define better stopping criteria? all bins explained? no longer helping remainder?
    num_bins_processed = 0
    while True:
        # this is the next max prob, after previous RFs subtracted when they help remainder
        max_prob_bin_index = np.argmax(np.sum(input_data_set, axis=0))

        # compute which RF index is best candidate for appending this bin index
        # TODO we need to deal with remainder adding back previous subtracted version of RF to update
        # TODO this gets pretty complicated
        rf_index_to_update, new_rf_value = _get_rf_to_update(rf_patterns, max_prob_bin_index, remainder_data_set, rf_values)

        if new_rf_value > rf_values[rf_index_to_update]:
            # update the RF if it is actually an improvement to add this bin (accounting for allowing overlap etc.)

            rf_patterns[rf_index_to_update, max_prob_bin_index] = 1
            rf_values[rf_index_to_update] = new_rf_value

            num_bins_processed += 1

            # TODO subtract updated pattern matches from remainder

        else:
            print('breaking at num bins process: ', num_bins_processed)
            break

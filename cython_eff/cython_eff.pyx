...
def compute_eff(np.ndarray[np.float64_t, ndim=2] weights,
                np.ndarray[np.float64_t, ndim=2] remainder,
                np.ndarray[np.float64_t, ndim=1] eff_output):
                #np.ndarray[np.float64_t, ndim=1] sum_w_output):

    cdef np.int32_t num_rf, cols, row, col
    # cdef np.float64_t tmp_sum, tmp_sum_w

    num_rf = weights.shape[0]
    cols = weights.shape[1]

    # for row in range(num_rf):
    for row in prange(num_rf,  nogil=True, schedule='static', num_threads=8):
        for col in range(cols):
            eff_output[row] += fabs(weights[row, col] - remainder[0, col])
            #sum_w_output[row] += fabs(weights[row, col])

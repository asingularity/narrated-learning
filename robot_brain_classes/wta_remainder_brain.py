...
class WTARemainderBrain(object):
    ...
    def _process_hyperlayer(self, hl_input, hl):
        ...
        if False:  # this fails because symmetry never broken
            for k in range(top_k):
                ...
                other_rf_indices = np.concatenate((top_k_indices[0:k], top_k_indices[k+1::]))

                # should not just sum here
                # ideally: should use separate w_rem, thresholded remainder; and only that should be subtracted as remainder
                tmp_diff = hl_input - self.weights[hl][other_rf_indices, :]
                tmp_diff_abs = np.abs(tmp_diff)
                tmp_argmin = np.argmin(tmp_diff_abs, axis=0)
                #print(tmp_argmin.shape)  # (1536,)
                #print(tmp_diff.shape)  # (3, 1536)
                weight_target = tmp_diff[tmp_argmin, np.arange(tmp_diff.shape[1])]
                #print(weight_target.shape, weight_target)
                #exit(1)
                #weight_target = tmp_diff[:, tmp_argmin)

                weight_target[weight_target < -1] = -1
                weight_target[weight_target > 1] = 1

                lr = self.lr_base

                self.weights[hl][win_rf_index, :] = (1.0 - lr) * self.weights[hl][win_rf_index, :] + lr * weight_target
        ...
        if False:
            for iter_i in range(self.num_iter[hl]):
                ...
                if hl==0 or self.t > self.start_time_per_hl[hl] + delta_t_start_per_iter * iter_i:
                    hl_output[win_rf_index] = 1.0  # old way

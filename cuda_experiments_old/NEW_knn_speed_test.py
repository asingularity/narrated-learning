...
def cuda_init(data, query_data):
    ...
    mod = SourceModule("""
        __global__ void knn_query(float *data, float *arr, long *min_indices, float *min_dists)
        {
          ...
          // stride over length of data[] instead, which is dim * 4000000
    ...
    return data_gpu, arr_gpu, min_dists_gpu, min_dists_tmp, min_indices_gpu, min_indices_tmp, func


#@profile
def cuda_query(query_data, cuda_data_gpu, cuda_arr_gpu, func, b_doubled, data, min_dists_gpu, min_dists_tmp, min_indices_gpu, min_indices_tmp):
...
def run_cuda_test(test_seconds):
    ...
    cuda_data_gpu, cuda_arr_gpu, min_dists_gpu, min_dists_tmp, min_indices_gpu, min_indices_tmp, cuda_func = cuda_init(data, query_data)
    ...
    for frame in range(test_frames):
        query_data = np.random.random(dim).astype(np.float32)
        dist, ind = cuda_query(query_data, cuda_data_gpu, cuda_arr_gpu, cuda_func, b_doubled, data, min_dists_gpu, min_dists_tmp, min_indices_gpu, min_indices_tmp)

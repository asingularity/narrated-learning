...
class RobotSensors(object):
    ...
    def read_input(self, robot_environment):
        ...
        for k in range(relative_ray_radians.shape[0]):
            ...
            # TODO ray_theta needs normalization to same range as theta. potentially circular for some tiles!
            # TODO for that we need to see what min, max of precomputed thetas are
            ...
            if matching_tile_indices.shape[0] > 0:
                ...
                self.ray_colors[k] = matching_tile_colors[np.argmin(matching_tile_dists)]
            else:
                self.ray_colors[k] = 0
